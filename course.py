...
SUBJECT_COURSES_API = (
    "https://pitcsprd.csps.pitt.edu/psc/pitcsprd/EMPLOYEE/SA/s/"
    "WEBLIB_HCX_CM.H_COURSE_CATALOG.FieldFormula.IScript_SubjectCourses?institution=UPITT"
    "&subject={subject}"
)
COURSE_DETAIL_API = (
    "https://pitcsprd.csps.pitt.edu/psc/pitcsprd/EMPLOYEE/SA/s/"
    "WEBLIB_HCX_CM.H_COURSE_CATALOG.FieldFormula.IScript_CatalogCourseDetails?institution=UPITT"
    "&course_id={id}&effdt=2018-06-30&crse_offer_nbr=1&use_catalog_print=Y"
)
COURSE_SECTIONS_API = (
    "https://pitcsprd.csps.pitt.edu/psc/pitcsprd/EMPLOYEE/SA/s/"
    "WEBLIB_HCX_CM.H_BROWSE_CLASSES.FieldFormula.IScript_BrowseSections?institution=UPITT&campus="
    "&location=&course_id={id}&institution=UPITT&term={term}&crse_offer_nbr=1"
)
SECTION_DETAILS_API = (
    "https://pitcsprd.csps.pitt.edu/psc/pitcsprd/EMPLOYEE/SA/s/"
    "WEBLIB_HCX_CM.H_CLASS_SEARCH.FieldFormula.IScript_ClassDetails?institution=UPITT&term={term}"
    "&class_nbr={id}"
)
# id -> unique course ID, not to be confused with course code (e.g., CS 0007 has code 105611)
...
def get_section_details(term: str | int, class_number: str | int) -> tuple[Course, Section]:
    term = _validate_term(term)
    ...
    if len(meetings) != 0:
        ...
        for meeting in meetings:
            ...
            instructors = None
            if len(meeting["instructors"]) != 0 and meeting["instructors"][0]["name"] not in [
                "To be Announced",
                "-",
            ]:
                instructors = []
    ...
    return Course(
        subject_code=details["subject"],
        course_number=details["catalog_nbr"],
